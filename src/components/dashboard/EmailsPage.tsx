...
import {
  ...
  Loader2, RefreshCw, Trash2, Check, Settings,
        ...
        <div className="p-6 flex flex-col gap-4 overflow-y-auto">
          {!initial && (
            <div className="rounded-xl p-3 text-xs text-slate-500" style={{ backgroundColor: "#f0f5ff", border: "1px solid #dbeafe" }}>
              ...
              <p><span className="font-semibold">Namecheap:</span> mail.privateemail.com / mail.privateemail.com</p>
          ...
          <ModalField label="Display Name" value={label} onChange={setLabel} placeholder="e.g. Office 101" />
          <ModalField label="Email Address" value={email} onChange={setEmail} placeholder="info@yourdomain.com" type="email" />

          <div className="grid grid-cols-3 gap-3">
            <div className="col-span-2"><ModalField label="IMAP Host" value={imapHost} onChange={setImapHost} placeholder="mail.privateemail.com" /></div>
            <ModalField label="IMAP Port" value={imapPort} onChange={setImapPort} placeholder="993" />
          </div>

          <div className="grid grid-cols-3 gap-3">
            <div className="col-span-2"><ModalField label="SMTP Host" value={smtpHost} onChange={setSmtpHost} placeholder="mail.privateemail.com" /></div>
      ...
      <div className="flex flex-col items-center justify-center h-full gap-6 p-8" style={{ backgroundColor: "#f8fafc" }}>
        ...
        <div className="text-center">
          ...
          <p className="text-sm text-slate-400 mt-2 max-w-xs leading-relaxed">
            Connect any IMAP email account to read and reply to emails from here.
          </p>
        </div>
            ...
            accounts.map(acc => {
              ...
              return (
                <button key={acc.id} onClick={() => setActiveId(acc.id)}
                  ...
                  {unread > 0 && <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full text-white" style={{ backgroundColor: col }}>{unread}</span>}
                  {isActive && (
              ...
              <div className="flex-1 min-h-0 overflow-y-auto p-3 flex flex-col gap-2 bg-slate-50/40">
                {filtered.length === 0 ? (
                  <div className="flex flex-col items-center justify-center h-48 gap-3 text-center p-4">
                    <div className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-400"><Mail size={18} /></div>
                    <div>
